	...
	try {
	    ...
	    stage('Build Project (Static Code Analysis, Compile, JUnit, Code Coverage)') {
				...
				def M2_Home = tool name: 'maven_3_6_0', type: 'maven'
	       		sh "${M2_Home}/bin/mvn clean surefire-report:report jacoco:prepare-agent package jacoco:report sonar:sonar -DskipTestssts"
	       	}
	    }
	    ...
	    stage('Build Docker Image') {
	    		sh 'docker build -t bishnukc/admin-service:${BUILD_NUMBER} -t bishnukc/admin-service:latest -f Dockerfile .'
	    }
	        
	    stage('Push Docker Image to Docker Hub') { 
	    		withCredentials([usernamePassword(credentialsId: 'bishnukc', passwordVariable: 'pass', usernameVariable: 'user')]) {
	    			sh 'docker login -u $user -p $pass' 
	        		sh 'docker push bishnukc/admin-service'
			}	 
	    }
